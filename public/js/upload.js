...
//Listen for output_people
    /*socket.on("output_people", function(data) {
     if(data.length){
...
//main chat screen
    ...
    $("#msgs").on('click', function() {
        alert("you want Remove the message");
        $("div").remove("#messages");
    });
...
//  if (data.length !== 0) {
//    $("#msgs").append("<li><strong><center><span class='text-warning'>Last 10 messages:</li>");
//    $.each(data, function(data, msg) {
//      $("#msgs").append("<li><strong><i><center><span class='text-warning'>" + msg + "</span></li>");
//    });
//  } else {
//    $("#msgs").append("<li><strong><center><span class='text-warning'>No past messages in this room.</li>");
//  }
//});
    ...
    socket.on("update-people", function(data){
        //var peopleOnline = [];
        $("#people").empty();
        $('#people').append("<strong><li class=\"list-group-item active\" style='color: green'>People online<span class=\"badge\">"+data.count+"</span></li>");
        $.each(data.people, function(a, obj) {
            if (!("country" in obj)) {
                html = "";
            } else {
                html = "<img class=\"flag flag-in"+obj.country+"\"/>";
            }
            $('#people').append("<li class=\"list-group-item\"><span>" + obj.name + "</span> <i class=\"fa fa-"
            + obj.device + "\">" + "</i> " + html + " <a href=\"#\" class=\"whisper btn btn-xs\">whisper</a></li>");
    ...
    socket.on("chat", function(person, msg) {
        $("#msgs").append("<div id='messages'><strong><span class='text-success'>" + person.name
        + "</span></strong><li id='add'> " + msg +"</li>" +"<small>"+"<i>"+(new Date().toString("h:mm tt"))+"<br>"+"</div>");
        //clear typing field
        $("#"+person.name+"").remove();
        clearTimeout(timeout);
    ...
    $(document).ready(function() {
        $('.form-logout').on('click', function () {
            if (typeof(Storage) !== "undefined") {
                // Store
                localStorage.setId("li", "#add");
                // Retrieve
                document.getElementById("#msgs").innerHTML = localStorage.getItem("li");
                $("li").show("#conversation");
            } else {
                document.getElementById("#msgs").innerHTML = "Sorry, your browser does not support Web Storage...";
            }
        });
